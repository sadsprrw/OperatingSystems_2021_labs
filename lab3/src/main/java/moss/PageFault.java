...
public class PageFault {
    ...
    public static void replacePage(Vector mem, int virtPageNum, int replacePageNum, ControlPanel controlPanel) {
        ...
        int bestNRUPage = -1;
        Vector<Vector<Integer>> classes = new Vector<>();
        for (int i = 0; i < 4; i++)
            classes.add(new Vector<>());

        for (int i = 0; i < virtPageNum; i++) {
            Page page = (Page) mem.elementAt(i);
            if (page.physical != -1) {
                if ((page.R == 0) && (page.M == 0))
                    classes.elementAt(0).add(i);
                else if ((page.R == 0) && (page.M == 1))
                    classes.elementAt(1).add(i);
                    else if ((page.R == 1) && (page.M == 0))
                        classes.elementAt(2).add(i);
                        else classes.elementAt(3).add(i);
            }
        }

        for(int i = 0; i < 4; i++){
            Vector<Integer> pClass = classes.elementAt(i);
            if(!pClass.isEmpty()){
                bestNRUPage = pClass.firstElement();
                break;
            }
        }

        Page page = (Page) mem.elementAt(bestNRUPage);
        Page nextpage = (Page) mem.elementAt(replacePageNum);
        controlPanel.removePhysicalPage(bestNRUPage);
